...
def main():
    ...
    checks = {
        ...
        "Paper Plots (Step 6)": PAPER_PLOTS.exists() and any(PAPER_PLOTS.glob("*.png")),
    }
    ...
    if twin_df is not None:
        st.sidebar.info(
            f"**Dataset:** {len(twin_df):,} rows  \n"
            f"**Batteries:** {twin_df['battery'].nunique()}  \n"
            f"**Cycles:** {twin_df.groupby('battery')['cycle'].nunique().sum()}"
        )
